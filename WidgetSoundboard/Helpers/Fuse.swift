...
public class Fuse {
    ...
    public typealias FusableSearchResult = (index: Int,
                                            score: Double,
                                            results: [(key: String,
                                                       score: Double,
    ...
    public init(location: Int = 0, distance: Int = 100, threshold: Double = 0.6, maxPatternLength: Int = 32,
    ...
    public func search(_ pattern: Pattern?,
                       in aString: String) -> (score: Double, ranges: [CountableClosedRange<Int>])? {
        ...
        if self.tokenize {
            ...
            let averagedResult = (score: results.score / Double(wordPatterns.count + 1),
    ...
    private func _search(_ pattern: Pattern,
                         in aString: String) -> (score: Double, ranges: [CountableClosedRange<Int>]) {
        var text = aString
        ...
        for i in 0 ..< pattern.len {
            ...
            while binMin < binMid {
                if FuseUtilities.calculateScore(pattern.len, e: i, x: location, loc: location + binMid,
            ...
            for j in (start ... finish).reversed() {
                ...
                let charMatch: Int = {
                    if currentLocation < textCount {
                        currentLocationIndex = currentLocationIndex.map { text.index(before: $0) } ?? text
                            .index(text.startIndex, offsetBy: currentLocation)
                ...
                if (bitArr[j] & pattern.mask) != 0 {
                    score = FuseUtilities.calculateScore(pattern.len, e: i, x: location, loc: currentLocation,
                                                         distance: distance)
            ...
            if FuseUtilities
                .calculateScore(pattern.len, e: i + 1, x: location, loc: location, distance: distance) > threshold {
                break
            }
...
public extension Fuse {
    ...
    func search(_ text: String, in aList: [String], chunkSize: Int = 100,
    ...
    func search(_ text: String, in aList: [Fuseable]) -> [FusableSearchResult] {
        ...
        for (index, item) in aList.enumerated() {
            ...
            collectionResult.append((index: index,
    ...
    ///   - chunkSize: The size of a single chunk of the array. For example, if the array has `1000` items, it may be useful to split the work into 10 chunks of 100. This should ideally speed up the search logic. Defaults to `100`.
    ///   - completion: The handler which is executed upon completion
    func search(_ text: String, in aList: [Fuseable], chunkSize: Int = 100,
        ...
        stride(from: 0, to: count, by: chunkSize).forEach { offset in
            ...
            self.searchQueue.async {
                ...
                for (index, item) in chunk.enumerated() {
                    ...
                    chunkResult.append((index: offset + index,
                                        score: totalScore / Double(scores.count),
                                        results: propertyResults))
                }
...
extension String {
    ...
    func lastIndexOf(_ searchStr: String, position: Int? = 0) -> String.Index? {
        ...
        let r = Range<Index>
            .init(uncheckedBounds: (lower: self.startIndex,
